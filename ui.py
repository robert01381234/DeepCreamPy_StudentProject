...
class PaintApplication(framework.Framework):
    ...
    def on_save_menu_clicked(self, event=None):
         if self.file_name == 'untitled':
            self.on_save_as_menu_clicked()
             ...
             decensorer.save_image(combined_img.convert('RGB'), self.file_name)
             
             
             messagebox.showinfo(
                     "Decensoring", "Save complete! image saved to {save_path}".format(save_path=self.file_name))
             self.file_name = orig_name
        
    def on_save_as_menu_clicked(self):
        saved_name = filedialog.asksaveasfilename(
    ...
    def on_decensor_menu_clicked(self, event=None):
        ...
        im02 = Image.open(self.file_name)
        im02.show()
        self.file_name = orig_name
